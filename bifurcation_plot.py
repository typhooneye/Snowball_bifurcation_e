...
adv_flux = st.sidebar.slider("adv_flux (Advection flux/S0)", 0.0, 1.0, 0.05, 0.01)

# Instantiate the class with updated parameters
SB = Critical_Insolation(ao=ao, ai=ai, phi_c_deg=phi_c_deg, A=A, B=B, T_f=T_f, k_i=k_i, adv_flux=adv_flux)
S_ice_free = SB.F_s_ice_free()
e_arr = np.linspace(0.005,1.0,50)
